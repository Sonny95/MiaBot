  ...
  const handleSend = async (message = input) => {
    if (message.trim()) {
      setMessages((prevMessages) => [
        ...
        { text: "", user: "MIA", loading: true }, // Placeholder for bot response
      ]);
      setInput("");
      setIsLoading(true);
      ...
      try {
        ...
        if (response.ok) {
          ...
          setMessages((prevMessages) =>
            prevMessages.map((msg, index) => {
              if (msg.loading) {
                return { text: data.message, user: "MIA" };
              }
              return msg;
            })
          );
        ...
        setMessages((prevMessages) =>
          prevMessages.map((msg, index) => {
            if (msg.loading) {
              return { text: "Failed to send message", user: "MIA" };
            }
            return msg;
          })
        );
    ...
    <div className="w-full max-w-md p-4 bg-gray-800 text-white rounded-lg transition-all duration-500 ease-in-out">
      ...
      <div className="flex space-x-2">
        <input
          ...
          disabled={isLoading}
        />
        <button className="p-2 bg-blue-500 rounded-lg" onClick={handleSend} disabled={isLoading}>
          {isLoading ? (
            <svg
